...
export const removeCart = (product) => {
    return (dispatch,getState) => {
        const cartItems = getState().cart.cartItems;
        const cartItemsClone = [...cartItems];
        const updeatedCartItems = cartItemsClone.filter( p => p._id !== product._id )
        dispatch({
            type: REMOVE_CART,
            data:{
               cartItems:updeatedCartItems }
        });
        localStorage.setItem('cartItems', JSON.stringify(updeatedCartItems))
    }
}
